...
import JobDetailHeader from '@/app/components/job/JobDetailHeader';
import Toast from '@/app/components/profile/Toast';
import { Heart, X } from 'lucide-react';
import { useRouter, useSearchParams } from 'next/navigation';
import { useAuth } from '@/contexts/AuthContext';
import { getJobDetail, type JobFromAPI } from '@/utils/api/job-api';
import { CandidateApi, type ApplyJobDto } from '@/utils/api/candidate-api';

/**
 * Candidate JobDetail Page
 * This page is for candidates to view job details and apply for jobs
 * Uses public/candidate APIs only
 */
function JobDetailContent_Inner() {
  ...
  const [confirmAction, setConfirmAction] = useState<'apply' | 'unapply' | 'save' | null>(null);
  ...
  // Fetch job detail from public API
  useEffect(() => {
    const fetchJobDetail = async () => {
      ...
      try {
        ...
        // Use public API to get job detail
        const response = await getJobDetail(jobId);
        const apiJob: JobFromAPI = response.data;
        ...
        // Check if candidate has applied (if logged in)
        if (user) {
  ...
  const checkApplicationStatus = async () => {
    ...
    try {
      ...
      const hasAppliedToJob = applications.some(app => app.jobId === jobId);
  ...
  const confirmHandler = async () => {
    ...
    try {
      ...
      if (confirmAction === 'apply') {
        // Apply for job using default CV
        const applyData: ApplyJobDto = {};
        await CandidateApi.applyJob(token, jobId, applyData);
        setHasApplied(true);
        showToast('Ứng tuyển thành công!');
      } else if (confirmAction === 'unapply') {
        // Unapply for job (if API exists)
        // For now, just show a message
        showToast('Tính năng hủy ứng tuyển đang được phát triển', 'error');
      } else if (confirmAction === 'save') {
        // Toggle saved job status
        const result = await CandidateApi.toggleSavedJob(token, jobId);
        setIsSaved(result.saved);
        showToast(result.saved ? 'Đã lưu công việc' : 'Đã bỏ lưu công việc');
      }
    } catch (error: any) {
  ...
  const getModalContent = () => {
    switch (confirmAction) {
      ...
      case 'save':
        return {
          title: isSaved ? 'Xác nhận bỏ lưu' : 'Xác nhận lưu công việc',
          message: isSaved 
            ? 'Bạn có chắc chắn muốn bỏ lưu công việc này không?' 
            : 'Bạn có chắc chắn muốn lưu công việc này không?',
          confirmText: isSaved ? 'Bỏ lưu' : 'Lưu',
